...
def analysis_folder():
    with open(pkg_resources.resource_filename(__name__, '../config.yaml'),
              'r') as f:
        return  yaml.safe_load(f)['analysis_folder']
...
def set_rcParams(_plt, transparency=True):
...
def slim_margins(cbar=False):
    if cbar:
        raise Exception('need to implement')
        return dict(
            left=0.3125 * dpi, right=0.4861 * dpi, top=0.2778 * dpi,
            bottom=0.2778 * dpi)
    return dict(left=90, right=45, top=55, bottom=70)
...
def color_bar(ax, im, label=None, fontsize=5, labelpad=12, width=20,
              spacing=0.15, height='100%', bottom_offset=0.):
    axins = inset_axes(
        ax, width=width/ax.figure.dpi, height=height, loc='lower left',
        bbox_to_anchor=(1 + spacing, bottom_offset, 1, 1), bbox_transform=ax.transAxes,
        borderpad=0)
...
def annotation(ax, x, y, text, fontsize=5, va='center'):
    ax.annotate('', (x[0], y), xytext=(x[1], y),
                ...
                arrowprops={'arrowstyle': '-', 'shrinkA': 0,
                            'shrinkB': 0}, clip_on=False)

    ax.text((x[0] + x[1]) / 2, y, text, ha='center', va=va,
            fontsize=fontsize)

def annotation_horiz(ax, y, x, text, fontsize=5, ha='center'):
    ax.annotate('', (x, y[0]), xytext=(x, y[1]),
                textcoords='data',
                arrowprops={'arrowstyle': '-', 'shrinkA': 0,
                            'shrinkB': 0})
    ax.text(x, (y[0] + y[1]) / 2, text, ha=ha, va='center',
...
def plot_seg_colors(ax, x, y, c, y2=None, n=15, gamma=1, cmap=None, **kwargs):
    if cmap is None:
        ...
        ls_cmap = matplotlib.colors.LinearSegmentedColormap.from_list('tmp_list', [c1, c2, c3], N=n, gamma=gamma)
    else:
        ls_cmap = cmap
    ...
    if y2 is not None:
        y2_formated = np.append(y2, [np.nan] * (n-len(y2)%n)).reshape(n, -1).T
        y2_formated = np.concatenate((y2_formated, np.expand_dims(np.append(y2_formated[0, 1:], [np.nan]), 0)))
        for i, (x_form, y_form, y2_form) in enumerate(zip(x_formated.T, y_formated.T, y2_formated.T)):
            ax.fill_between(x_form, y_form, y2_form, **kwargs)
    else:

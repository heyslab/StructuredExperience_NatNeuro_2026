...
Extended Data Figure 5 is generated by:

```text
ms_figures/fp_analysis/error_trials_figure.py  # Extended Data Figure 5a,b,c,d,e,f
```

This script analyzes error-like RNN trajectories and their relationship to
fixed/slow-point structure in the trained models.
...
Extended Data Figure 6 scripts are in `ms_figures/fp_analysis/` and
`ms_figures/`:

```text
ms_figures/fp_analysis/limit_flows_pca_figure.py  # Extended Data Figure 6a,b
ms_figures/fp_analysis/response_gating.py         # Extended Data Figure 6c
ms_figures/s_cue_test.py                          # Extended Data Figure 6d
```

These scripts extend the fixed-point and limit-cycle analyses from Figure 4 and
test model responses to isolated cue inputs.

The fixed-point scripts use trained model paths and metadata from the companion
`models_database` release, along with precomputed fixed-point files stored next
to each trained model as:
...
They also use the fixed-point/RNN helper code included with the Figure 4
analysis scripts:
...
Extended Data Figure 8 combines representative histology images with mouse
behavior and MEC decoding analyses. The Python scripts included here generate
the behavioral raster, ephys tangling, and SL-decoder confusion-matrix panels:

```text
ms_figures/ephys_behvior/allbehavior_lickraster.py       # Extended Data Figure 8c,d
ms_figures/ephys_time_decoding/tangling_figure.py        # Extended Data Figure 8e
ms_figures/ephys_time_decoding/test_ephys_decode.py      # Extended Data Figure 8f,g
```

The histology panels in Extended Data Figure 8a,b are representative image
panels from the manuscript and are not generated by the included Python
analysis scripts.

The ephys tangling panel uses precomputed per-session tangling files:

```text
analysis/tangling.h5
```

The script used to generate those files from each recording session's
`spikes.h5` file is included as:

```text
ms_figures/ephys_time_decoding/tangling_ephys.py
```

Similarly, the SL-decoder confusion matrices reuse the per-session time
decoding files described for Figure 6:
...
Extended Data Figure 9 scripts are in `ms_figures/fp_analysis/`:

```text
ms_figures/fp_analysis/dynamic_motifs_omit_one.py  # Extended Data Figure 9a
ms_figures/fp_analysis/dynamic_motifs_probes.py    # Extended Data Figure 9b
```

These scripts extend the fixed-point and dynamical-motif analyses from
Figure 4 to probe how the trained RNN strategy generalizes to modified cue
configurations. `dynamic_motifs_omit_one.py` tests trials where the first cue
is omitted, while `dynamic_motifs_probes.py` tests untrained cue configurations
such as LL, MM, and extended cue-duration probe trials.

Both scripts use trained model paths and metadata from the companion
`models_database` release and expect precomputed fixed-point files stored next
to each trained model:

```text
fps_info.pkl
```

They also reuse the fixed-point/RNN helper code included with the Figure 4
analysis scripts:
...
The required probe-task trial generators are defined in:

...
def rnn_params(h0, b, w_in, w_r, w_out, b_out, gamma, unit_noise):
  ...
  Arguments:
    ...
    w_in: weights for the input layer
    w_r: recurrent weights
    w_out: weights for the output layer
    b_out: bias for the output layer
    gamma: leak factor
    unit_noise: private unit noise level

  Returns:
    a dictionary of parameters
  """
  return {'h0' : h0, 'b': b, 'w_in': w_in, 'w_r': w_r, 'w_out': w_out,
          'gamma': gamma, 'unit_noise': unit_noise, 'b_out': b_out}
...
def rnn(params, h, x):
  """Implement the leaky RNN equations.
  ...
  i_t = np.dot(x, params['w_in']) + np.dot(h, params['w_r']) + params['b']
  return ((1 - params['gamma']) * h + params['gamma'] * np.tanh(i_t))


def rnn_scan(params, h, x):
...
def affine(params, x):
  ...
  Args: 
    ...
    x: np array of input"""
  return np.dot(x, params['w_out']) + params['b_out']
...
def rnn_run_with_h0(params, x_t, h0):
  ...
  f = partial(rnn_scan, params)
  _, h_t = lax.scan(f, h0, x_t)
  o_t = batch_affine(params, h_t)
  return h_t, o_t  


def rnn_run(params, x_t):
  """Run the Vanilla RNN T steps, where T is shape[0] of input.
  
  Args:
    params: dict of RNN parameters
    x_t: np array of inputs with dim ntime x u

  Returns: 
    2-tuple of np arrays (hidden states w dim ntime x n, outputs w dim ntim x o)
  """  
  return rnn_run_with_h0(params, x_t, params['h0'])

  
# Let's make it handle batches using `vmap`
batched_rnn_run = vmap(rnn_run, in_axes=(None, 0))
batched_rnn_run_w_h0 = vmap(rnn_run_with_h0, in_axes=(None, 0, 0))

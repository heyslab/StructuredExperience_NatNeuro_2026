...
def main(args):
    ...
    jP.set_rcParams(plt)
    dpi = 300
    path = Path('/analysis/ms_figures/fps_analysis_probes')
    ...
    for model_type, color, color2, ls in zip(('no_shaping', 'only_shaping', 'shaping'),
        ...
        for task in ('include_long_match', 'include_med_match', 'super_long'):
            trials_gen = genFactory.create(
                task, input_noise=0, batch_size=1, n_blocks=1)
            inputs = trials_gen.generate_trials(1)[['light', 'odor']]
            inputs = inputs.groupby(['type', 'idx']).head(1).swaplevel(1, 0)
            ...
            for arrow_loc in arrow_locs:
                ...
                axs[1].annotate('', null_pcs[arrow_loc, [0, 2]], null_pcs[arrow_loc-3, [0, 2]],
                                arrowprops=arrowprops, zorder=zorder)
                axs[2].annotate('', (null_pcs[arrow_loc, 0], null_out[arrow_loc]), (null_pcs[arrow_loc-1, 0], null_out[arrow_loc-3]),
                                arrowprops=arrowprops, zorder=zorder)
            ...
            for arrow_loc in arrow_locs:
                ...
                axs[1].annotate('', null_pcs[arrow_loc, [0, 2]], null_pcs[arrow_loc-3, [0, 2]],
                                arrowprops=arrowprops, zorder=zorder)
                axs[2].annotate('', (null_pcs[arrow_loc, 0], null_out[arrow_loc]), (null_pcs[arrow_loc-3, 0], null_out[arrow_loc-3]),
                                arrowprops=arrowprops, zorder=zorder)
            ...
            if trial_type == 'super_long':
                axs[0].set_title('XL', color=TRIAL_COLORS[trial_type])
            else:
